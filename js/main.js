...
function iniciarVideo(){
  ...
  document.getElementById('sello-iniciales').textContent = C.pareja.iniciales;
...
function pintarUbicacion(){
  ...
  document.getElementById('map-label').textContent = C.lugar.nombre;
}

// ---------------- ADDONS: GALERÍA Y FIRMAS ----------------
function pintarAddons(){
  const seccionGaleria = document.getElementById('section-galeria');
  if (!C.addons || !C.addons.galeria) {
    ...
    document.getElementById('input-foto').addEventListener('change', subirFotoCloudinary);
  }

  const seccionFirmas = document.getElementById('section-firmas');
  if (!C.addons || !C.addons.libroFirmas) {
...
async function subirFotoCloudinary(e){
  const archivo = e.target.files[0];
  if (!archivo) return;
  const { cloudName, uploadPreset } = C.cloudinary;
  if (!cloudName || cloudName.startsWith('PEGAR_')) { alert('Falta configurar Cloudinary en config.js'); return; }
  const formData = new FormData();
  formData.append('file', archivo);
  formData.append('upload_preset', uploadPreset);
  formData.append('transformation', 'f_jpg,q_auto');
  try {
    ...
    if (data.secure_url) {
      ...
      div.innerHTML = `<img src="${data.secure_url}" alt="">`;
...
async function enviarFirma(e){
  ...
  if (!apiKey || apiKey.startsWith('PEGAR_')) { alert('Falta configurar Airtable en config.js'); return; }
  try {
    await fetch(`https://api.airtable.com/v0/${baseId}/${tableId}`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ fields: { Nombre: nombre, Mensaje: mensaje } })
    });
...
async function cargarFirmas(){
  const { apiKey, baseId, tableId } = C.airtable;
  if (!apiKey || apiKey.startsWith('PEGAR_')) return;
  try {
    const res = await fetch(`https://api.airtable.com/v0/${baseId}/${tableId}`, { headers: { 'Authorization': `Bearer ${apiKey}` } });
    const data = await res.json();
...
function iniciarCountdown(){
  ...
  function actualizar(){
    ...
    el.innerHTML = `${bloque(dias,'Días')}<span class="cd-dot">:</span>${bloque(horas,'Horas')}<span class="cd-dot">:</span>${bloque(min,'Min')}<span class="cd-dot">:</span>${bloque(seg,'Seg')}`;
  }
...
function iniciarReveal(){
  const obs = new IntersectionObserver((entries) => {
    entries.forEach(e => { if (e.isIntersecting) e.target.classList.add('visible', 'in-view'); });
  }, { threshold: 0.15 });
